...
public class RndServiceImplV1 {
	...
	// 객체지향에서 다형성(중복정의)
	// 한클래스 내에서는 같은 이름의 메서드를 쓸수 없다
	// 하지만 매개변수를 갖는게 다르면 중복해서 쓸 수 있다.
	/*
	 * 원칙은 객체지향의 (메서드)다형성
	 * 한 클래스 파일내에서는 같은 이름의 method를 두번이상 정의할 수 없다
	 * 하지만 객체지향 언어에서는 
	 * method의 매개변수가 있거나 없거나 
	 * 또는 매개변수의 개수가 다르거나 
	 * 매개변수의 type이 다르면 
	 * 같은 이름의 method를 중복하여 정의할 수 있다(중복정의)
	 * 
	 ...
	 * 코드들에서 오류가 나타나버렸다.
	 * 
	 *  이 오류를 객체지형의 다형성 원리를 이용하여 해결하였다
	 *  
