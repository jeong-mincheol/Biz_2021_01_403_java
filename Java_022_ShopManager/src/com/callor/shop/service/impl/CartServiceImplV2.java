...
public class CartServiceImplV2 extends CartServiceImplV1 implements CartServiceV2{
	...
	/*
	 * cartList에 저장된 상품리스트를 .../cartlist.txt파일에 저장
	 * 
	 * 어떤 데이터를 파일에 저장하기 
	 * 1. 파일이름(경로포함)을 결정하고 
	 * 2. 파일을 생성
	...
	public void saveCartToFile() {
		 ...
		 *  	생성하고자 하는 파일의 각종 정보들에 문제가 발생하여 exception이 발생할 수 있다.
		 *  
		 *  	아무리 코드를 견고하게 작성하여도
		...
		try {
			...
			for(int i = 0 ; i < nSize ; i++) {
				...
				printer.printf("%s,%s,%d,%d,%d\n",
						cartVO.getUserName(),cartVO.getProductName(),
						cartVO.getQty(), cartVO.getPrice(), cartVO.getTotal());
			}
